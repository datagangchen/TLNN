...
class Always():
    ...
    def robustness(self, weight, rho):
        ...
        if min(wrho[self.tau1:self.tau2+1])>0:
            ...
            result = result**(1.0/(self.tau2-self.tau1+1)) -1
...
class AND():
    ...
    def reset_weight(self,threshold):
        rew =[]
    ...
    def gradient_r(self, j):
        ...
        if min(self.rho)>0:
            ...
            return signfunction(self.w[j]/M, False)
...
class OR():
    ...
    def output(self, weight, rho):
        self.rho = rho 
        self.w = weight
        evn = Eventually(weight, 0,len(self.rho)-1)
...
class TLNN():
    ...
    def gradient_descent(self, eta, x, yd):
        ...
        new_topand_w = [max(0.11,xr-eta*yr) for xr, yr in zip(weight, gradient)]  
